...
RequestRedisCache.prototype.get = function (params, cb) {
  ...
  var that = this;
  that.redis.get(cacheKey, function handleCacheGet (getErr, infoStr) {
    // Define fresh get function
    function freshGet() {
      params.uncachedGet(params.requestOptions, function handleUncachedRead (uncachedGetErr, info) {
        // If there was an error, callback with it
        if (uncachedGetErr) {
          return cb(uncachedGetErr);
        }

        // Save the original response
        var infoStr = that.stringify(info);
        that.redis.setex(cacheKey, cacheTtl, infoStr, function handleCacheSet (setexErr) {
          // If there was an error, emit it
          if (setexErr) {
            setexErr.action = 'RequestRedisCache#get: Could not save fresh data to redis';
            setexErr.cacheKey = cacheKey;
            setexErr.infoStr = infoStr;
            console.log('wat2', setexErr);
            that.emit('error', setexErr);
          }
    ...
    if (getErr) {
      // Emit the error
      getErr.action = 'RequestRedisCache#get: Could not get cached data from redis';
      getErr.cacheKey = cacheKey;
      console.log('wat3', getErr);
      that.emit('error', getErr);
    ...
    // Otherwise, if the item exists
      ...
      // If we cannot parse the cache
        ...
        // Emit the error
        err.action = 'RequestRedisCache#get: Could not parse cached data from redis';
        ...
        that.redis.del(cacheKey, function handleCacheInvalidation (err) {
          // If there was an error, emit it
          if (err) {
            err.action = 'RequestRedisCache#get: Could not delete un-parsable cached data on redis';
            err.cacheKey = cacheKey;
            err.infoStr = infoStr;
            that.emit('error', err);
          }
