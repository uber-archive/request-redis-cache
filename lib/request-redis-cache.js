...
RequestCache.prototype = {
  read: function (params, cb) {
    ...
    if (cacheKey === undefined) {
      return cb(new Error('`RequestCache#read` requires `params.cacheKey` but none was provided'));
    }
    var cacheTtl = params.cacheTtl;
    if (cacheTtl === undefined) {
      return cb(new Error('`RequestCache#read` requires `params.cacheTtl` but none was provided'));
    }
    var uncachedRead = params.uncachedRead;
    if (uncachedRead === undefined) {
      return cb(new Error('`RequestCache#read` requires `params.uncachedRead` but none was provided'));
    }
    var requestOptions = params.requestOptions;
    if (requestOptions === undefined) {
      return cb(new Error('`RequestCache#read` requires `params.requestOptions` but none was provided'));
    }
